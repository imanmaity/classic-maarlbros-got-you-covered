...
def parse_master(input_dir):
    ...
    if paths:
        ...
        # Merge additional schedule files, but the primary (largest = authoritative master)
        # OWNS its own dates: extra files may only fill dates the master does not cover.
        # So a recovered previous-week file contributes its non-overlapping past weeks, while
        # a stale/discarded file that overlaps the master contributes nothing on those days --
        # the master's cells always win (no unioning a superseded room/time back in).
        seen_m = {(m["abbr_norm"], m["division"], m["date"], m["session"]) for m in meetings}
        seen_e = {(e["date"], e["type"], e["name"]) for e in events}
        _covered_m = {m["date"] for m in meetings}       # dates the master governs
        _covered_e = {e["date"] for e in events}
        for p in paths[1:]:
            _, g2 = _read_sheets(p)
            if not g2: continue
            *_, m2, e2 = _build_master(cd_rows, g2, os.path.basename(p))   # reuse catalog for a consistent alias map
            add_m = add_e = 0; _fm, _fe = set(), set()
            for m in m2:
                if m["date"] in _covered_m: continue                       # master (or an earlier file) owns this day
                k = (m["abbr_norm"], m["division"], m["date"], m["session"])
                if k not in seen_m: seen_m.add(k); meetings.append(m); _fm.add(m["date"]); add_m += 1
            for e in e2:
                if e["date"] in _covered_e: continue
                k = (e["date"], e["type"], e["name"])
                if k not in seen_e: seen_e.add(k); events.append(e); _fe.add(e["date"]); add_e += 1
            _covered_m |= _fm; _covered_e |= _fe                           # this file now owns the days it filled
            note("INFO", f"Merged extra schedule {os.path.basename(p)} (gap-fill only): +{add_m} meetings, +{add_e} events.")
